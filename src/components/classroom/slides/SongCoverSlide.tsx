// FriendlyTeaching.cl — Slide 1: Song Cover
'use client';
...
export default function SongCoverSlide({ slide }: Props) {
  ...
  return (
    <div className="relative flex flex-col h-full min-h-[480px] overflow-hidden bg-gradient-to-br from-[#1E0F35] via-[#3D1F6B] to-[#2D1B4E] text-white">

      {/* Album art — full blurred background */}
      {imgSrc && (
        ...
        <img
          ...
          alt=""
          aria-hidden
          className="absolute inset-0 w-full h-full object-cover opacity-20 blur-xl scale-110 pointer-events-none select-none"
        />
      )}
      ...
      <div className="relative flex flex-col items-center justify-center flex-1 px-8 py-10 gap-6">
        ...
        <div className="flex items-center gap-2">
          ...
          />
          <span className="text-xs font-extrabold uppercase tracking-[0.25em] text-[#C8A8DC]">
            Friendlyrics®
          </span>
        </div>
        ...
        <div className="text-center">
          <h1 className="text-3xl font-extrabold leading-tight drop-shadow-lg mb-2">
            {song?.title ?? slide.title ?? 'Music Lesson'}
          </h1>
          <p className="text-xl text-white/70 font-medium">
            {song?.artist ? `by ${song.artist}` : slide.subtitle ?? ''}
          </p>
        </div>
