...
export default function SpeakingSlide({ slide, isTeacher }: Props) {
  ...
  return (
    <div className="flex flex-col h-full overflow-auto p-6 items-center justify-center text-center">
      ...
      <h2 className="text-3xl font-bold text-[#5A3D7A] mb-2">{slide.title}</h2>

      {slide.content && (
        <p className="text-xl text-gray-600 mb-6 max-w-lg">{slide.content}</p>
      )}
        ...
        <div className="space-y-3 w-full max-w-lg">
          {prompts.map((p, i) => (
            <div key={i} className="bg-[#F0E5FF]/60 border border-[#C8A8DC]/30 rounded-2xl px-5 py-4 text-left">
              <span className="text-sm font-bold text-[#9B7CB8] mr-2">#{i + 1}</span>
              <span className="text-lg text-gray-800 font-medium">{p}</span>
            </div>
          ))}
        </div>
      )}
