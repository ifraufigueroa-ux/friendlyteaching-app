...
export default function ClipLanguageFocusSlide({ slide }: Props) {
  ...
  // Two-column layout when we have examples to show alongside the context;
  // fall back to the original single-column centred layout otherwise.
  const twoCol = examples.length > 0;

  // Pattern cards: stack vertically inside the left column (space is
  // constrained). In single-column mode, spread horizontally like before.
  const gridCols = twoCol
    ? 'grid-cols-1'
    : bullets.length <= 1 ? 'grid-cols-1'
    : bullets.length === 2 ? 'grid-cols-1 sm:grid-cols-2'
    : bullets.length === 3 ? 'grid-cols-1 sm:grid-cols-3'
    : 'grid-cols-1 sm:grid-cols-2';

  const contextBlock = (
    <div className={`flex flex-col gap-6 ${twoCol ? 'items-start text-left' : 'items-center text-center'}`}>
      {/* Eyebrow + hero icon */}
      <div className={`flex flex-col gap-3 ${twoCol ? 'items-start' : 'items-center'}`}>
      ...
      {/* Title */}
      <h1 className={`font-serif font-bold text-[#2D1B4E] leading-tight ${twoCol ? 'text-3xl md:text-4xl' : 'text-4xl md:text-5xl max-w-3xl'}`}>
        {titleText}
      </h1>

      {/* Intro paragraph */}
      {intro && (
        <p className={`text-base text-[#5A3D7A]/85 leading-relaxed ${twoCol ? '' : 'md:text-lg max-w-2xl'}`}>
          {intro}
        </p>
      )}
            ...
            {bullets.map((b, i) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="font-mono font-bold text-[#5A3D7A] text-sm md:text-base leading-tight pt-2">
                    {pattern}
                  </p>
                  {explain && (
      ...
      {outro && (
        <p className={`text-sm text-[#5A3D7A]/70 leading-relaxed ${twoCol ? '' : 'max-w-2xl'}`}>
          {outro}
        </p>
      )}
      ...
      {slide.teacherNotes && (
        <div className={`w-full bg-amber-50/80 border border-amber-200 rounded-2xl px-5 py-3 text-sm text-amber-800 leading-relaxed ${twoCol ? '' : 'max-w-2xl'}`}>
  ...
  return (
    <div className="relative h-full overflow-y-auto bg-gradient-to-br from-[#F9F5FF] via-[#FBF8F0] to-[#FFE8F0] text-[#2D1B4E]">
      ...
      {twoCol ? (
        <div className="max-w-7xl mx-auto px-[2cm] py-[2cm] grid grid-cols-1 lg:grid-cols-[minmax(0,7fr)_minmax(0,5fr)] gap-10 lg:gap-14 items-start">
          {contextBlock}
          {examplesBlock}
        </div>
      ) : (
