// FriendlyTeaching.cl — Planner: lesson-preparation kanban tab (extracted from
// the original /planner page). Same UX as before — global kanban board with a
// per-student modal that reads/writes student-specific plan overrides.
'use client';
...
// ── Main kanban tab export ─────────────────────────────────────────────────────

export default function LessonKanbanTab({ teacherId }: { teacherId: string }) {
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="overflow-x-auto p-6 pt-2">
        <div className="flex gap-4 min-h-[500px]" style={{ minWidth: `${COLUMNS.length * 240}px` }}>
          {COLUMNS.map(col => (
            <KanbanColumn
              key={col.id}
              column={col}
