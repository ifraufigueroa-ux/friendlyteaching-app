  ...
  useEffect(() => {
    ...
    if (mode === 'resetPassword') {
      ...
      applyActionCode(auth, oobCode)
        ...
        .catch(() => {
          setPageState('error');
          setErrorMsg('No se pudo verificar el correo. El enlace puede haber expirado.');
        });
    } else {
      setPageState('error');
    ...
    <div className="min-h-screen flex items-center justify-center p-4"
      ...
      <Suspense fallback={
        <div className="bg-white/80 rounded-2xl p-12 text-center">
          <div className="w-8 h-8 border-2 border-[#C8A8DC] border-t-[#5A3D7A] rounded-full animate-spin mx-auto" />
        </div>
      }>
        <AuthActionContent />
      </Suspense>
