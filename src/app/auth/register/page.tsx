...
export default function RegisterPage() {
  ...
  const handleGoogleRegister = async () => {
    if (isTeacher) return; // Teachers must use email/password with teacher code
    setError('');
    setGoogleLoading(true);
    try {
      const result = await signInWithGoogle();
      if (result.isNewUser && result.role === 'student') {
        ...
        if (teacherEmail) {
          fetch('/api/notify', {
            ...
            body: JSON.stringify({
              ...
              to: teacherEmail,
              studentName: result.user.displayName,
        ...
        {!isTeacher && (
          <>
            ...
            <button
              ...
              className="w-full mt-3 flex items-center justify-center gap-3 px-4 py-3 bg-white/80 hover:bg-white border border-white/60 rounded-xl text-sm font-semibold text-gray-700 shadow-sm transition-all hover-lift disabled:opacity-60"
            >
              {googleLoading ? (
