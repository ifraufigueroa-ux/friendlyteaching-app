// FriendlyTeaching.cl — Friendlytext CLT Teacher Dashboard
...
function AssignModal({
  ...
  async function save() {
    setSaving(true);
    await assignTextLesson(lesson.id!, selected);
    onClose();
  }
...
export default function TextsPage() {
  ...
  async function togglePublish(lesson: TextLesson) {
    await publishTextLesson(lesson.id!, lesson.publishStatus !== 'published');
  }

  async function handleDelete(lesson: TextLesson) {
    if (!confirm(`¿Eliminar la lección "${lesson.title}"? No se puede deshacer.`)) return;
    await deleteTextLesson(lesson.id!);
  }
      ...
      <TopBar
        ...
        actions={
          <button
            ...
            className="flex items-center gap-2 px-3 py-2 bg-gradient-to-r from-[#1B2C3F] to-[#4B6A85] text-white rounded-full text-xs font-bold shadow-md hover:shadow-lg flex-shrink-0"
          >
            ＋ Crear text lesson
          </button>
        }
      ...
      <div className="max-w-6xl mx-auto mt-4">
        {loading ? (
          ...
          <div className="bg-white rounded-2xl p-12 shadow-sm text-center">
            ...
            <p className="text-[#1B2C3F] font-bold text-lg mb-1">Aún no hay text lessons</p>
            <p className="text-gray-400 text-sm mb-5">Pega un texto, elige el audio (o genera TTS), y deja que la IA arme el deck CLT.</p>
            <button
              onClick={() => setShowCreate(true)}
              className="px-5 py-2.5 bg-gradient-to-r from-[#1B2C3F] to-[#4B6A85] text-white rounded-full text-sm font-bold shadow"
            >
              ＋ Crear text lesson
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {lessons.map(lesson => {
              const thumb = lesson.text?.youtubeUrl ? ytThumbnail(lesson.text.youtubeUrl) : lesson.text?.posterUrl ?? null;
              const isPub = lesson.publishStatus === 'published';
              const hasAudio = Boolean(lesson.text?.audioUrl || lesson.text?.youtubeUrl);
              const audioLabel = lesson.text?.audioSource === 'tts'
                ? '🎙 TTS'
                : lesson.text?.audioSource === 'youtube' || lesson.text?.youtubeUrl
                  ? '▶ YouTube'
                  : lesson.text?.audioSource === 'hosted'
                    ? '🔊 Hosted'
                    : '📖 Silent';
              return (
                <div key={lesson.id} className="bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 flex flex-col">
                  ...
                  >
                    {thumb ? (
                      ...
                      <img src={thumb} alt={lesson.text?.title ?? ''} className="w-full h-full object-cover opacity-80 transition-transform group-hover:scale-105" />
                    ) : (
                    ...
                    <span className="absolute bottom-2 left-2 text-[10px] font-bold text-white bg-black/60 border border-white/20 px-2 py-0.5 rounded-full">
                      {audioLabel}
                    </span>
                  </button>
                  <div className="p-4 flex-1 flex flex-col">
                    <div className="flex items-start justify-between gap-2 mb-1">
                      <h3 className="font-bold text-gray-800 text-sm leading-snug line-clamp-2">{lesson.text?.title ?? lesson.title}</h3>
                    ...
                    <p className="text-xs text-gray-400 truncate mb-2">{lesson.text?.source ?? '—'}</p>
                    <div className="flex flex-wrap gap-2 text-[10px] text-gray-500 mb-3">
                      <span className="bg-gray-100 px-2 py-0.5 rounded-full">📝 {lesson.slides?.length ?? 0} slides</span>
